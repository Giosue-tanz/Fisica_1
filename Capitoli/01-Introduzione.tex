...
\subsection{Propagazione dell'Errore: Esempio Geometrico}
...
\begin{figure}[h]
    ...
    \end{tikzpicture}
    \caption{Propagazione dell'errore nel calcolo dell'Area ($A \pm \delta A$).}
...
\begin{tcolorbox}[colback=yellow!5, colframe=orange!75!black, title=\textbf{Rivoluzione della Metrologia (2019)}]
...
\subsection{Esempio Applicativo: Il Pendolo Semplice}
...
\begin{figure}[h]
    ...
    \end{tikzpicture}
    \caption{Pendolo semplice di lunghezza $l$ e massa $m$.}
\end{figure}
...
\[
\begin{cases}
    \alpha = 0 & (\text{esponente di } M) \\
    \beta + \gamma = 0 & (\text{esponente di } L) \\
    -2\gamma = 1 & (\text{esponente di } T)
\end{cases}
\]

\textbf{Soluzione del sistema:}
\begin{teorema}{Analisi Dimensionale del Pendolo}
...
\section{I Moti della Cinematica}
...
\begin{itemize}
    ...
    \item Moto armonico smorzato (esponenzialmente)
    \item Moto di caduta libera verticale
    \item Moto parabolico di caduta libera
    \item Moto circolare uniforme
    \item Moto circolare vario
\end{itemize}

Molti di questi (i primi elencati e quelli di caduta verticale) appartengono alla famiglia dei \textbf{moti unidimensionali}, ovvero avvengono lungo una sola retta.
...
\begin{itemize}
    \item \textbf{Moto Rettilineo Uniforme}: $v(t) = \text{costante}$, $x(t) = x_0 + v(t-t_0)$. Acceleratione $a=0$.
    \item \textbf{Moto Rettilineo Uniformemente Accelerato}: $a(t) = \text{costante}$.
    \[ v(t) = v_0 + a(t-t_0); \quad x(t) = x_0 + v_0(t-t_0) + \frac{1}{2}a(t-t_0)^2 \]
    \begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title=\textbf{Formula di Torricelli (Senza il tempo)}]
    ...
    \textbf{Dimostrazione:}
    Partiamo dalle due equazioni del moto uniformemente accelerato:
    \begin{enumerate}
        \item $v = v_0 + at \quad \Rightarrow \quad t = \frac{v - v_0}{a}$
        \item $x = x_0 + v_0 t + \frac{1}{2} a t^2$
    \end{enumerate}
    Sostituiamo $t$ dalla (1) nella (2):
    \[ x - x_0 = v_0 \left(\frac{v - v_0}{a}\right) + \frac{1}{2} a \left(\frac{v - v_0}{a}\right)^2 \]
    \[ x - x_0 = \frac{v_0 v - v_0^2}{a} + \frac{1}{2} a \frac{v^2 - 2v v_0 + v_0^2}{a^2} \]
    \[ x - x_0 = \frac{2v_0 v - 2v_0^2 + v^2 - 2v v_0 + v_0^2}{2a} \]
    \[ x - x_0 = \frac{v^2 - v_0^2}{2a} \quad \Rightarrow \quad 2a(x - x_0) = v^2 - v_0^2 \]
    \end{tcolorbox}
\end{itemize}

Nel \textbf{Moto Armonico Semplice}, invece, l'accelerazione $a(t)$ \textbf{NON è costante}.
...
\begin{tcolorbox}[colback=blue!5, colframe=blue!75!black, title=Determinismo e Teorema di Cauchy]
...
\begin{align}
    x(t) &= A \left(1 - e^{-\frac{t}{\tau}}\right) \\
    v(t) &= \frac{A}{\tau} e^{-\frac{t}{\tau}} \\
    a(t) &= -\frac{A}{\tau^2} e^{-\frac{t}{\tau}}
\end{align}
...
\begin{minipage}{0.4\textwidth}
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        % y = x - x^2/4 => @x=2 y=2-1=1. Let's scale better.
        % y = 2x - 0.5x^2 => @x=2 y=4-2=2. @x=4 y=8-8=0. Perfect.
